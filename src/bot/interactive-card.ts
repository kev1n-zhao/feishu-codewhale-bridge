 ...
 *      SDK only walks `elements`, so without intervention Codewhale sees the
 ...
 *      raw JSON so Codewhale can see the card structure.
 *
 * All branches wrap output in an `<interactive_card>` block. Codewhale is
...
export function expandInteractiveCard(
  ...
  // nodes). Substitute raw JSON so Codewhale can see the structure.
