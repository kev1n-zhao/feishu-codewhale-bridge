...
async function intakeMessage(deps: IntakeDeps): Promise<void> {
  ...
  const scopes = await resolveScopes(channel, msg.chatId, msg.threadId, chatModeCache);
  const chatMode = scopes.chatMode;
  const scope = scopes.sessionScope;
...
async function runAgentBatch(deps: RunBatchDeps): Promise<void> {
  ...
  const cwd = workspaces.cwdForProject(projectScope) ?? homedir();
  ...
  // For non-card modes Codewhale's output doesn't surface visually until either
...
async function processAgentStream(
  ...
  // Idle watchdog: codewhale going silent for `idleTimeoutMs` is treated as
  // "presumed hung", we stop() and surface a timeout marker on the card.
  //
  // BUT — codewhale can legitimately be silent for a long time when it's
  ...
  // case there's no event stream activity from codewhale itself, only the
  ...
  try {
    for await (const evt of handle.run.events) {
      ...
      await flush(state);
      // Stop iterating as soon as we have a terminal state. Some codewhale
  ...
  // wins. This avoids "codewhale finished but flush was slow → timer fired
  ...
  //  - Natural done: stream-json emits `result` ~1ms before codewhale actually
...
/**
 * How long to wait for codewhale to close stdout after a terminal event before
 * forcing a SIGTERM. Empirically codewhale's post-`result` tail is well under a
