...
export async function handleCommentMention(deps: CommentDeps): Promise<void> {
  ...
  // One Codewhale session per cloud-doc; subsequent @-mentions in the same
  ...
  try {
    ...
    for await (const e of run.events) {
      ...
      // as we have the final result. Some codewhale versions hang briefly post-
    ...
    if (errorMsg) reply = `⚠️ Codewhale 报错：${errorMsg}`;
...
function buildCommentPrompt(target: ResolvedTarget, ctx: CommentContext): string {
  // Construct a doc URL Codewhale can hand to lark-cli. The exact subdomain
