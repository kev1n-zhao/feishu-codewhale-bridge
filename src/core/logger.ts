...
const LOG_RETENTION_DAYS = Math.max(
  1,
  Number(process.env.LARK_CODEWHALE_LOG_DAYS ?? 7) || 7,
);
 ...
 *  1. JSON line into `~/.lark-codewhale/logs/YYYY-MM-DD.log` — the durable
 ...
 * the local machine — specifically, before /doctor feeds it to Codewhale (the
 * Anthropic API will see it) and before the analysis card lands in a
 * Feishu chat (the Lark server may cache card contents).
 *
 * Conservative: keeps log structure intact so Codewhale can still correlate by
