...
export class WorkspaceStore {
  ...
  cwdForProject(projectScope: string): string | undefined {
    return this.data.chats[projectScope]?.cwd;
  }

  setProjectCwd(projectScope: string, cwd: string): void {
    this.data.chats[projectScope] = { cwd };
    this.schedulePersist();
  }

  cwdFor(chatId: string): string | undefined {
    return this.cwdForProject(chatId);
  }

  setCwd(chatId: string, cwd: string): void {
    this.setProjectCwd(chatId, cwd);
  }
