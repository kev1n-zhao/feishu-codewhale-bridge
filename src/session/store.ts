...
export class SessionStore {
  ...
  async load(): Promise<void> {
    try {
      ...
      for (const [chatId, entry] of Object.entries(raw)) {
        ...
        // would hang codewhale on a missing jsonl, so resume keys still need
   ...
   * Sessions recorded in a different cwd are stale — codewhale can't resume
  ...
  clearProject(projectScope: string): number {
    const prefix = `${projectScope}:`;
    let count = 0;
    for (const key of Object.keys(this.data)) {
      if (key === projectScope || key.startsWith(prefix)) {
