 ...
 * shape so lark-cli's `--source lark-codewhale` reads it through the same
 ...
 * `~/.lark-codewhale/config.json` (no CLI surface yet — by design, since
...
export interface AppPreferences {
   ...
   * output. Default true. Turn off if you only care about Codewhale's final
   * text answer and want to hide the "工具调用过程".
   */
  showToolCalls?: boolean;
  /**
   * Cap on concurrent codewhale runs across all chats / topics. Excess runs
  ...
  /**
   * Global default idle-timeout for codewhale runs, in minutes. When set,
   * if codewhale emits no stream event for this long the bridge kills the
   ...
   * message reach Codewhale (the 0.1.21-and-earlier behavior).
   *
  ...
  access?: AppAccess;
  /**
   * Grace period (ms) between SIGTERM and SIGKILL when killing the codewhale
   * subprocess. Bumped from a hardcoded 500ms because codewhale often has its
...
export function getMaxConcurrentRuns(cfg: AppConfig): number {
  ...
  // Reasonable upper bound — at 50+ concurrent codewhales the bot box is
 ...
 */
/**
 * Grace period before SIGKILL fallback when stopping a codewhale subprocess.
