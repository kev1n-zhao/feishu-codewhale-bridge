...
import { buildPrompt } from '../bot/prompt';

export interface Controls {
  /** Restart the bridge in-process: disconnect WS, kill codewhale runs, reload
...
export interface CommandContext {
  ...
  /**
   * Session scope string. Top-level chat messages use `msg.chatId`; any
   * threaded message uses `${chatId}:${threadId}` so each thread gets its
   * own session and active run. Workspace cwd is project-scoped, not
   * session-scoped.
   */
  scope: string;
  /** Project/workspace scope. One Feishu chat owns one cwd across threads. */
  projectScope: string;
  /** Resolved chat mode for `msg.chatId`. Used for reply threading and status. */
...
async function handleNewChat(rawName: string, ctx: CommandContext): Promise<void> {
  const sourceCwd = ctx.workspaces.cwdForProject(ctx.projectScope);
  ...
  if (sourceCwd) {
    ctx.workspaces.setProjectCwd(created.chatId, sourceCwd);
  }
...
async function handleCd(args: string, ctx: CommandContext): Promise<void> {
  ...
  const interrupted = ctx.activeRuns.interruptProject(ctx.projectScope);
  ctx.workspaces.setProjectCwd(ctx.projectScope, absolute);
  const cleared = ctx.sessions.clearProject(ctx.projectScope);
  log.info('command', 'cd-project', { projectScope: ctx.projectScope, interrupted, cleared });
...
async function handleWsList(ctx: CommandContext): Promise<void> {
  ...
  const currentCwd = ctx.workspaces.cwdForProject(ctx.projectScope);
...
async function handleWsSave(name: string, ctx: CommandContext): Promise<void> {
  ...
  const cwd = ctx.workspaces.cwdForProject(ctx.projectScope);
  if (!cwd) {
...
async function handleWsUse(name: string, ctx: CommandContext): Promise<void> {
  ...
  const interrupted = ctx.activeRuns.interruptProject(ctx.projectScope);
  ctx.workspaces.setProjectCwd(ctx.projectScope, cwd);
  const cleared = ctx.sessions.clearProject(ctx.projectScope);
  log.info('command', 'ws-use-project', { projectScope: ctx.projectScope, interrupted, cleared });
...
async function handleResume(args: string, ctx: CommandContext): Promise<void> {
  ...
  const cwd = ctx.workspaces.cwdForProject(ctx.projectScope) ?? homedir();
...
const DOCTOR_INSTRUCTIONS = `你是 lark-codewhale-bridge 的诊断助理。下面会给你两段输入:
1. 用户的故障描述
...
- phase: 模块阶段。常见值: ws(WebSocket), intake(消息入站), queue(去抖队列), flush(批处理), media(附件下载), prompt(prompt 组装), session(会话), agent(codewhale 子进程), card(卡片渲染), comment(文档评论), cardAction(卡片回调), command(斜杠命令), sdk(飞书 SDK 内部)
          ...
          card: {
            ...
            producer: async (ctrl) => {
              ...
              for await (const evt of handle.run.events) {
                ...
                await flush();
                // Don't wait for stdout to close — some codewhale versions hang
...
async function handleDoc(args: string, ctx: CommandContext): Promise<void> {
  ...
  if (!request) {
    await reply(
      ctx,
      '用法：`/doc <要生成或更新的文档内容>` 或 `/spec <规格说明需求>`\n' +
        '我会让 Codewhale 使用 lark-cli 创建/更新飞书云文档，并把链接发回当前会话。',
    );
    return;
  }
  ...
  ctx.activeRuns.interrupt(ctx.scope);
  const cwd = ctx.workspaces.cwdForProject(ctx.projectScope) ?? homedir();
  const resumeFrom = ctx.sessions.resumeFor(ctx.scope, cwd);
  const docTask = [
    '请把下面需求整理成飞书云文档。',
    '要求：',
    '- 使用已绑定的 lark-cli 创建或更新 Feishu/Lark Docx。',
    '- 只在当前 bridge_context.chat_id / thread_id 对应会话里回发文档链接。',
    '- 如果 lark-cli 不可用、未登录、权限不足或创建失败，明确说明失败原因和下一步，不要假装已创建。',
    '- 文档内容用中文，结构清晰，可直接作为规格/方案继续协作。',
    '',
    '用户需求：',
    request,
  ].join('\n');
  const prompt = buildPrompt(
    [{ ...ctx.msg, content: docTask, resources: [] }],
    [],
    [],
    {
      projectScope: ctx.projectScope,
      sessionScope: ctx.scope,
      replyMode: getMessageReplyMode(ctx.controls.cfg),
    },
  );
  ...
  try {
    await ctx.channel.stream(
        ...
        card: {
          ...
          producer: async (ctrl) => {
            ...
            for await (const evt of handle.run.events) {
              if (handle.interrupted) break;
              if (evt.type === 'system') {
                if (evt.sessionId) ctx.sessions.set(ctx.scope, evt.sessionId, evt.cwd ?? cwd);
                continue;
              }
      ...
      { replyTo: ctx.msg.messageId, ...(ctx.chatMode === 'topic' && ctx.msg.threadId ? { replyInThread: true } : {}) },
    );
  } catch (err) {
    log.fail('command', err, { step: 'doc' });
  } finally {
    ctx.activeRuns.unregister(ctx.scope, run);
  }
}
...
async function submitAccount(ctx: CommandContext): Promise<void> {
  ...
  void (async () => {
    ...
    // raw secret. lark-cli's `config bind --source lark-codewhale` reads the
