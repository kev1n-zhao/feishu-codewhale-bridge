...
export interface StatusInfo {
  ...
  agentName: string;
  /** Project/workspace scope (= chatId). */
  projectScope: string;
...
export function statusCard(info: StatusInfo): object {
  const sessionLine = info.sessionId
    ...
    : '(无)';
  // Surface both layers: cwd follows the project scope, conversation history
  // follows the session scope.
  const scopeLine =
    info.scope !== info.projectScope
      ? `\`${escapeCode(info.scope)}\` _（thread 独立 session）_`
      : `\`${escapeCode(info.scope)}\``;
  const lines = [
    `🧭 **project**: \`${escapeCode(info.projectScope)}\``,
    `💬 **session**: ${scopeLine}`,
    `📁 **cwd**: \`${escapeCode(info.cwd)}\``,
    `🔗 **codewhale**: ${sessionLine}`,
  ...
  return shell('💡 使用帮助', [
    divMd(
      [
        ...
        '- `/doctor [描述]` — 把日志和描述喂给 Codewhale 自助诊断',
        '- `/doc <需求>` `/spec <需求>` — 让 Codewhale 创建/更新飞书云文档并回发链接',
        '- `/help` — 本帮助',
        '',
        '其他内容直接交给 Codewhale。',
      ].join('\n'),
