...
const MANUAL_INSTALL_HINT = [
  ...
  `  ${BOLD}lark-cli config bind --source lark-codewhale --identity bot-only${RESET}`,
  '',
...
async function checkLarkCli(opts: PreFlightOptions): Promise<void> {
  ...
  console.log(
    [
      ...
      'ℹ️  lark-cli 未安装',
      '',
      'lark-cli 是飞书的命令行工具,装上后 Codewhale 可以:',
      '  • 主动发送交互卡片 / 表单',
      '  • 查询日历、文档、待办、OKR、考勤',
      '  • 200+ 飞书 API 命令',
      '',
  ...
  const bindResult = await runCapture(
    'lark-cli',
    ['config', 'bind', '--source', 'lark-codewhale', '--identity', 'bot-only'],
    BIND_TIMEOUT_MS,
  );
...
function printInstallFailedWarning(): void {
  console.error(
    [
      ...
      '',
      'Bridge 仍会继续启动,但 Codewhale 工具调用会受限。',
      '请手动执行:',
      '',
      `  ${BOLD}npm install -g @larksuite/cli${RESET}`,
      `  ${BOLD}lark-cli config bind --source lark-codewhale --identity bot-only${RESET}`,
