...
/**
 * Poll `~/.lark-codewhale/processes.json` for a freshly-registered bridge
 ...
 * definition file.
 *
 * Idempotent. Leaves ~/.lark-codewhale/ state untouched (keystore, sessions,
...
export async function runServiceUnregister(): Promise<void> {
  ...
  console.log('  (配置 / 日志 / 会话保留在 ~/.lark-codewhale/)');
}
